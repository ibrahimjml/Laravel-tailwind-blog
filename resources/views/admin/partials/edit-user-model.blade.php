<div id="editModel-{{ $user->id }}" class="hidden fixed w-2/6 z-[20] max-h-[80%] overflow-y-auto py-8 left-[50%]  top-[50%] transform translate-x-[-50%] translate-y-[-50%] items-center space-y-2 font-bold bg-gray-700 rounded-lg drop-shadow-lg border border-gray-300 transition-all duration-300">
 ...
 <form id="phone-form-{{ $user->id }}" action="{{route('update.user',$user->id)}}" method="POST">
 @csrf
 @method("PUT")
 <label for="name" class="mt-2 block text-slate-200 text-sm mb-1 font-bold  ">
Name:
</label>
 <input  type="text" value="{{ old('name', $user->name) }}" class="block  w-72 rounded-lg p-2 border-2 text-white  bg-transparent @error('name') border-red-500 @enderror"
  name="name" >
  @error('name')
...
Username:
 ...
 <input  type="text" value="{{ old('username', $user->username) }}" class="block  w-72 rounded-lg p-2 border-2 text-white  bg-transparent @error('username') border-red-500 @enderror"
  name="username" >
  @error('username')
      <p class="text-red-500 text-xs italic mt-4">
          {{ $message }}
...
Email:
 ...
 <input  type="text" value="{{ old('email', $user->email) }}" class="block  w-72 rounded-lg p-2 border-2 text-white  bg-transparent @error('email') border-red-500 @enderror"
  name="email" >
  @error('email')
      <p class="text-red-500 text-xs italic mt-4">
          {{ $message }}
...
<input  type="number" value="{{ old('age', $user->age) }}" class="block  w-72 rounded-lg p-2 border-2 text-white  bg-transparent @error('age') border-red-500 @enderror"
name="age" >
@error('age')
<p class="text-red-500 text-xs italic mt-4">
  {{ $message }}
...
<input id="phone-{{ $user->id }}" type="tel"
value="{{ old('phone', $user->phone) }}"
 class="block  w-72 rounded-lg p-2 border-2 text-white  bg-transparent @error('phone') border-red-500 @enderror"
...
Password:
 ...
 <input  type="password" class="block  w-72 rounded-lg p-2 border-2 text-white  bg-transparent @error('password') border-red-500 @enderror"
  name="password" >
  @error('password')
      <p class="text-red-500 text-xs italic mt-4">
          {{ $message }}
...
Confirm password:
 ...
 <input  type="password" class="block  w-72 rounded-lg p-2 border-2 text-white  bg-transparent @error('password_confirmation') border-red-500 @enderror"
...
<div class="flex flex-col h-52 overflow-y-auto">
    <label for="roles" class="mt-2 block text-slate-200 text-sm mb-1 font-bold">Permissions:</label>
  @foreach ($permissions as $permission)  
    <label class="mr-4 text-white">
      <input type="checkbox" name="permissions[]" value="{{ $permission->id }}" class="mr-1" {{ $user->hasPermission($permission->name) ? 'checked' : '' }}>
      {{ $permission->name }}
    </label>
  @endforeach
</div>
...
@push('scripts')
  <script>
      document.addEventListener('DOMContentLoaded', function () {
        const phoneInputField = document.querySelector("#phone-{{ $user->id }}");
...
fetch('https://get.geojs.io/v1/ip/geo.json')
        ...
        const form = document.querySelector(`#phone-form-{{ $user->id }}`);
