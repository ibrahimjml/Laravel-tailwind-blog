<div id="Model" class="hidden fixed inset-0 z-30 bg-black bg-opacity-50 flex items-center justify-center p-4">
 ...
 <form id="addcategory" action="{{route('admin.categories.create')}}" method="POST" class="space-y-6">
 @csrf
 @method("POST")
  <!-- Form Fields -->
        <div>
          <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Category:</label>
          <input type="text" name="name" placeholder="type a category" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('name') border-red-500 @enderror">
