<div id="bgmodel" class="fixed hidden inset-0 z-50 opacity-0 transition-opacity duration-300 ease-in-out">
  ...
  <div id="commentModel"
    class="relative h-[100vh] w-screen md:w-[40%] overflow-x-hidden bg-white overflow-y-auto shadow-xl translate-x-[-110vw] transition-all duration-300 ease-in-out">
    <span id="closeModel" title="close" class="cursor-pointer absolute top-4 right-4 text-xl"><i
    ...
    <!-- Load More ajax -->
    <x-load-more button-id="load-more-comments"
                 container="comments-container" 
                 :route="route('posts.comments.load', $post)"
                 page-name="comments_page" 
                 :current-page="$comments->currentPage()" 
                 :has-more="$comments->hasMorePages()"
