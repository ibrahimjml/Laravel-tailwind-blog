<x-layout>
  ...
  <span
    class="flex items-center gap-4 p-2 rounded-lg bg-black/70 text-xl font-bold text-gray-300 mb-2 ml-4 pb-1  w-fit  my-6">
          ...
          <div class="swiper-wrapper">
            @foreach($latestNews as $news)
              <div class="relative swiper-slide">
                <a href="{{ $news->link }}" rel="nofollow" class="block">
                  ...
                  <span class="absolute top-2 left-2 bg-black/40 text-gray-300 p-1 rounded-lg text-sm"><i
                      class="fas fa-clock mr-2"></i>{{ $news->created_at->diffForHumans() }} </span>
                </a>
              </div>
            @endforeach
          </div>
    ...
    <div class="lg:w-1/3 w-full flex flex-col gap-3 ">
      ...
      @foreach($moreNews as $more)
        <div class="border p-4 w-full rounded-xl bg-card shadow-sm flex gap-4 hover:shadow-md transition-shadow">
          <img class="w-24 h-24 object-cover rounded-md border shrink-0" src="{{ $more->image_url }}" alt="Preview">
          ...
          <div class="flex-1 min-w-0 flex flex-col">
            ...
            <div class="flex justify-between mt-auto pt-3 border-t">
              <a href="{{ $more->link }}" target="_blank" class="text-xs text-blue-500 hover:underline">
                {{ $more->source->name }}
              </a>
        ...
        <article
          ...
          <div class="p-4">
            <div class="mb-3 flex items-center justify-between text-sm text-gray-500">
              <p class="font-medium">{{$post->totalcomments_count}} {{Str::plural('comment', $post->totalcomments_count)}}
              </p>
              <p>{{$post->created_at->format('F d, Y')}}</p>
            </div>
            <a href="{{ route('single.post', $post->slug) }}" class="block">
              <p class="text-base font-semibold text-slate-800 leading-snug">{{ $post->title }}</p>
            </a>
          </div>
        </article>
      @endforeach
        ...
        <article
          ...
          <div class="flex items-center gap-3 p-4">
            <a href='{{route('profile.home', $latest->user->username)}}'>
              <img loading="lazy" src="{{$latest->user->avatar_url}}" alt="{{ $latest->user->name }}'s profile"
          ...
          <div class="p-4">
            ...
            <a href="{{route('single.post', $latest->slug)}}" class="block">
              <p class="text-base font-semibold text-slate-800 leading-snug">{{$latest->title}}</p>
            </a>
          </div>
        </article>
      @endforeach
