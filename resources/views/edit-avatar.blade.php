...
<div class="flex justify-center ">
  ...
  <div class="absolute z-10">
    <form action="{{route('delete.avatar',$user)}}" method="POST">
      @csrf
      @method("DELETE")
  ...
  @endcan
  @endif
  <form action="/edit-avatar/{{auth()->user()->id}}/edit" method="POST" enctype="multipart/form-data" class="p-6">
    @csrf
    @method('PUT')
     
    <span class=" w-[180px] h-[180px] flex justify-center items-center ml-20">
      @if(auth()->user()->avatar !== "default.jpg")
      <img  src="{{Storage::url(auth()->user()->avatar)}}" alt=""  class="relative w-full h-full overflow-hidden flex justify-center items-center  shrink-0 grow-0 rounded-full border-4 border-blue-500">
      @else
